    ...
    return `
        <div class="admin-top">
            ...
            <div class="order-filter-bar">
                <label class="order-filter-label" for="orderFilter">
                    Order status
                </label>

                <select
                    id="orderFilter"
                    class="admin-search order-status-filter"
                >
                    <option value="all">
                        All Orders
                    </option>
...
function adminOrderRow(o, serial) {
    const displayStatus =
        o.status === "pending" ? "Incoming" :
        o.status === "confirmed" ? "Confirmed" :
        o.status === "delivered" ? "Delivered" :
        o.status === "cancelled" ? "Returned" :
        o.status || "Incoming";
        ...
        <tr
            data-search="${esc(
                `${serial} ${o.id} ${
                    ...
                    o.customer?.phone || ""
                } ${o.address || ""} ${displayStatus} ${
                    o.courierStatus || ""
                }`.toLowerCase()
            )}"
            data-status="${esc(displayStatus.toLowerCase())}"
        >

            <td>
                <b>#${serial}</b>
                <small>${dateVal(o.createdAt)}</small>
            </td>
            ...
            <td>
                <select
                    ...
                    data-id="${o.id}"
                    aria-label="Courier status for order #${serial}"
                >
                    <option value="Not sent" ${
                        !o.courierStatus ||
                        o.courierStatus === "Not sent"
                            ? "selected"
                            : ""
                    }>
                        Not sent
                    </option>
                    <option value="Sent to courier" ${
                        o.courierStatus === "Sent to courier"
                            ? "selected"
                            : ""
                    }>
                        Sent to courier
...
function bindAdminContent(route) {
    ...
    const applyOrderFilters = () => {
        ...
        $$("#ordersTbody tr").forEach(row => {
            const text =
                row.innerText.toLowerCase();

            const statusText =
                (row.dataset.status || text).toLowerCase();
            ...
            const matchesFilter =
                ...
                (
                    filter === "courier" &&
                    statusText.includes("sent to courier")
                ) ||
                (
                    filter === "delivered" &&
